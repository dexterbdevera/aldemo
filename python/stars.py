...
STARS = f"{API}/airac/stars/airport/"
...
ICAO_WAYPOINTS = {icao:{'STARSs':[]} for icao in icao_list}

for icao in icao_list:
    icao_data = requests.get(f"{STARS}{icao}", headers=HEADERS).json()
    if len(icao_data) > 0:
        for icao_attr in icao_data:
            ...
            ICAO_WAYPOINTS[icao]['STARSs'].append(icao_attr["name"])
            ICAO_WAYPOINTS[icao]['STARSs'] = list(set(ICAO_WAYPOINTS[icao]['STARSs']))

        # Rearrange Data
        TEMP3 = ICAO_WAYPOINTS[icao]['STARSs']

        del ICAO_WAYPOINTS[icao]['STARSs']
        TEMP1 = None
        ...
        ICAO_WAYPOINTS[icao] = {'WAYPOINT_COUNT': {TEMP1[TEMP2[0]]: TEMP2[0], TEMP1[TEMP2[1]]: TEMP2[1]}, 'STARSs': TEMP3}
        TEMP1 = None
        TEMP2 = None
        TEMP3 = None
